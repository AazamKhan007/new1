 ...
 */
export const handleCreateListing = async (req: AuthRequest, res: Response) => {
  try {
 ...
 */
export const handleGetListings = async (req: AuthRequest, res: Response) => {
  try {
    const userId = req.user?.id;
    if (!userId) {
      return res.status(401).json({ error: 'Unauthorized: User ID not found in token.' });
 ...
 */
export const handleUpdateListing = async (req: AuthRequest, res: Response) => {
  try {
    const userId = req.user?.id;
    if (!userId) {
 ...
 */
export const handleDeleteListing = async (req: AuthRequest, res: Response) => {
  try {
    const userId = req.user?.id;
    if (!userId) {
        return res.status(401).json({ error: 'Unauthorized: User ID not found.' });
 ...
 */
export const handleGetMyListings = async (req: AuthRequest, res: Response) => {
  try {
    const userId = req.user?.id;
    if (!userId) {
        return res.status(401).json({ error: 'Unauthorized: User ID not found in token.' });
 ...
 */
export const handleReportListing = async (req: AuthRequest, res: Response) => {
  try {
